#!/bin/bash
#
# Klipper TMC Autotune - Modified Installation Script
# Original: https://github.com/andrewmcgr/klipper_tmc_autotune
# Modified by: @woodworker (1/10/2026)
#
# MODIFICATION:
#   Removed Python version check to enable installation on ELEGOO Neptune 4 series
#   printers running the stock Python 2 Klipper image.
#
# IMPORTANT:
#   This script only performs the base installation (clone repo, create symlinks).
#   For Python 2 compatibility, you MUST run Neptune4Base.sh after this script:
#
#   Step 1: bash install.sh         (this file - installs base autotune)
#   Step 2: bash Neptune4Base.sh    (installs Python 2 compatibility layer)
#
# WHAT WAS CHANGED:
#   Lines 73-78: Python version check commented out
#   Original code blocked installation on Python 2 systems
#   Modified code allows installation to proceed regardless of Python version
#
# USE CASE:
#   - ELEGOO Neptune 4 / 4+ / 4 Pro / 4 Max with stock image
#   - Any Klipper system still running Python 2
#
# WARNING:
#   This is an ADVANCED modification not officially supported by ELEGOO or
#   the Klipper TMC Autotune project. Use at your own risk.
#
# SUPPORT:
#   https://github.com/cwiegert/Klipper-Autotune-Neptune-Base
#
###############################################################################
...
function preflight_checks {
   ...
   #    @woodworker -- 1/10/2026
   #        removed the check for the python version so this can run on the Elegoo base klipper.   Elegoo is running on 
   #        python2 and the TMC files are installed during the install on the woodworker git repo
